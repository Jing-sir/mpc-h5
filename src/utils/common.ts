...
export function onDownloadFile(val: string, name: string): void { // 下载公用方法
    const url = window.URL.createObjectURL(new Blob([val], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }));
...
export function buildTree<T extends Record<string, any>>(
    ...
    const tree: { [key: string]: T & { [childrenKey: string]: T[] } } = {};
    ...
    return array.reduce<T[]>((acc, item) => {
        ...
        tree[itemId] = currentItem;

        if (!parentId || parentId === '0') {
            acc.push(currentItem);
        } else {
            const parentItem = tree[parentId] as T & { [childrenKey: string]: T[] };
            if (parentItem) {
...
export function numberSeparation(num: number, separator = ','): string {
    ...
    return typeof num !== 'undefined'
        ? strNumber.replace(/\d+/, (s: string) =>
            s.replace(/(\d)(?=(\d{3})+$)/gim, `$1${separator}`),
        )
        : strNumber;
}
...
export const handlePaste = async (): Promise<string[] | []> => {
    ...
    if (!window.navigator.clipboard) {
        // message.warn(i18n.global.t('Copy_Warn_1'));
        throw new Error(i18n.global.t('Copy_Warn_2'));
    }

    try {
        const text = await window.navigator.clipboard.readText();
        // if (!SIX_NUMBER.test(text)) return message.warn(i18n.global.t('Copy_Warn_3'));
        // SIX_NUMBER.test(text)
...
export const transformUnicode2String = (str: string) => {
